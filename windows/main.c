...
DWORD WINAPI check_rows(LPVOID params);	//所有行行检查函数声明
DWORD WINAPI check_cols(LPVOID params);	//所有列检查函数声明
DWORD WINAPI check_square(LPVOID params);	//单个3*3子网格检查函数声明

int main(){
	int i, j, board[9][9];
	FILE *fp;	//建立外部输入文件sudoku_board的读取指针
	errno_t err = fopen_s(&fp, "sudoku_board", "r");	//vs2019要求使用fopen_s函数因为更安全
	for(i = 0; i < 9; i++){	//所有行
		for(j = 0; j < 9; j++){	//所有列
    ...
    HANDLE thread_rows, thread_cols, thread_square[9];	//声明所用线程（检查所有行、所有列、每个3*3网格）
    //初始化线程
    thread_rows = CreateThread(NULL, 0, check_rows, &param[0], 0, NULL);	//检查所有行，起点参数为[0, 0]
	thread_cols = CreateThread(NULL, 0, check_cols, &param[0], 0, NULL);	//检查所有列，起点参数为[0, 0]
	for(i = 0; i < 9; i++){	//检查九个3*3网格
		thread_square[i] = CreateThread(NULL, 0, check_square, &param[i], 0, NULL);
	}
    //等待线程运行完毕
	WaitForSingleObject(thread_rows, INFINITE);
	WaitForSingleObject(thread_cols, INFINITE);
	WaitForMultipleObjects(9, thread_square, TRUE, INFINITE);
    //检查标志，输出数独谜题解决是否有效
	if(flag == 1){	//无效
		printf("数独解决方案无效！\n");
	}else{	//有效
...
DWORD WINAPI check_rows(LPVOID params){
	...
	return 0;	//未出现重复数字，线程正常结束
}

//检查所有列是否都包含1-9的所有数字
DWORD WINAPI check_cols(LPVOID params){
    parameters * data = (parameters *) params;	//将结构体指针赋给data
    int startRow = data->row;	//取出行起点并赋给startRow
    int startCol = data->col;	//取出列起点并赋给startCol
...
DWORD WINAPI check_square(LPVOID params){
    parameters * data = (parameters *) params;	//将结构体指针赋给data
    int startRow = data->row;	//取出行起点并赋给startRow
    int startCol = data->col;	//取出列起点并赋给startCol
